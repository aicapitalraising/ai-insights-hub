...
import { ReferenceImagesManager } from '@/components/static-batch/ReferenceImagesManager';
import { toast } from 'sonner';
import {
  Plus, Palette, Loader2, Trash2, GripVertical, Pencil, Star, ImagePlus
} from 'lucide-react';
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (editingStyle) {
        ...
        const { error } = await supabase
          .from('ad_styles')
          .insert({ ...payload, display_order: maxOrder + 1, reference_images: [] });
        if (error) throw error;
        ...
        <Dialog open={createOpen} onOpenChange={(open) => { setCreateOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="space-y-4">
              <div>
                <Label>Name</Label>
                <Input value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="e.g. Bold Dark" />
              </div>
              <div>
                <Label>Description</Label>
                <Input value={formDescription} onChange={(e) => setFormDescription(e.target.value)} placeholder="Dark bg, bright text, high contrast" />
              </div>
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Generate a bold ad with dark background, vibrant accent colors..."
                  rows={3}
                />
              </div>
              <div>
            ...
            <Card key={style.id} className="relative group">
              ...
              <CardContent className="pt-0">
                ...
                <ReferenceImagesManager
                  ...
                  clientId={style.client_id || undefined}
                />
              </CardContent>
            </Card>
          ))}
