...
import {
  ...
  Trophy, ExternalLink, Play, Filter, Plus
} from 'lucide-react';
...
export function WinningAdsGallery({ embedded = false }: { embedded?: boolean }) {
  ...
  const handleDelete = useCallback(async (ad: WinningAd) => {
    const { error } = await supabase.from('custom_ads').delete().eq('id', ad.id);
    if (error) {
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Upload Winning Ads</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <Input
                  placeholder="e.g. Hook, Testimonial, Product Demo"
          ...
          {filtered.map((ad) => (
            ...
            >
              <div className="relative aspect-square bg-muted">
                ...
                {ad.category && (
                  <Badge className="absolute bottom-2 left-2 text-xs bg-yellow-500/90 text-black">
                    {ad.category}
                  </Badge>
                )}
              </div>
