...
import {
  ...
  Mail, MessageSquare, Megaphone, Video, Image, FileText, Globe, ChevronDown, ChevronUp
} from 'lucide-react';
...
export function OfferAssetHub({
  ...
  const generateAll = async () => {
    ...
    const { data: intakeData } = await supabase
      ...
      .order('created_at', { ascending: false })
      .limit(1);
    const intake = intakeData?.[0] || null;
    ...
    for (const assetType of ASSET_TYPES) {
      ...
      try {
        ...
        // Capture research and angles for downstream assets
